...
def getParkingEventsArray(id,interval,type = "lot" ,location = "MelbCity",start = None,end = None, output = "list",normalize = False):
    if location == "MelbCity":
        ...
        if interval == 1:
            if mps1mDF is None:
                mps1mDF =  pd.read_csv("./datasets/"+location+"/parking/"+type+"s/"+str(interval)+"m.csv")
            df = mps1mDF
        elif interval == 5:
            if mps5mDF is None:
                mps5mDF =  pd.read_csv("./datasets/"+location+"/parking/"+type+"s/"+str(interval)+"m.csv")
            df = mps5mDF
        elif interval == 15:
            if mps15mDF is None:
                mps15mDF =  pd.read_csv("./datasets/"+location+"/parking/"+type+"s/"+str(interval)+"m.csv")
            df = mps15mDF
